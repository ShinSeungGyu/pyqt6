## Ex 8-2. 직선 그리기 (drawLine).

import sys
from PyQt6.QtWidgets import QWidget, QApplication
from PyQt6.QtGui import QPainter, QPen
from PyQt6.QtCore import Qt


class MyApp(QWidget):

    def __init__(self):
        super().__init__()
        self.initUI()

    def initUI(self):
        self.setGeometry(300, 300, 400, 300)
        self.setWindowTitle('drawLine')
        self.show()

    def paintEvent(self, e):
        qp = QPainter()
        qp.begin(self)
        self.draw_line(qp)
        self.draw_line2(qp)
        qp.end()

    def draw_line(self, qp:QPainter):
        qp.setPen(QPen(Qt.GlobalColor.blue, 8))
        qp.drawLine(30, 230, 200, 50)
        qp.setPen(QPen(Qt.GlobalColor.green, 12))
        qp.drawLine(140, 60, 320, 280)
        qp.setPen(QPen(Qt.GlobalColor.red, 16))
        qp.drawLine(330, 250, 40, 190)

    def draw_line2(self, qp:QPainter):
        qp.setPen(QPen(Qt.GlobalColor.black, 3, Qt.PenStyle.SolidLine))
        qp.drawLine(20, 20, 380, 20)
        qp.drawText(30, 40, 'Qt.SolidLine')

        qp.setPen(QPen(Qt.GlobalColor.black, 3, Qt.PenStyle.DashLine))
        qp.drawLine(20, 70, 380, 70)
        qp.drawText(30, 90, 'Qt.DashLine')

        qp.setPen(QPen(Qt.GlobalColor.black, 3, Qt.PenStyle.DotLine))
        qp.drawLine(20, 120, 380, 120)
        qp.drawText(30, 140, 'Qt.DotLine')

        qp.setPen(QPen(Qt.GlobalColor.black, 3, Qt.PenStyle.DashDotLine))
        qp.drawLine(20, 170, 380, 170)
        qp.drawText(30, 190, 'Qt.DashDotLine')

        qp.setPen(QPen(Qt.GlobalColor.black, 3, Qt.PenStyle.DashDotDotLine))
        qp.drawLine(20, 220, 380, 220)
        qp.drawText(30, 240, 'Qt.DashDotDotLine')

        pen = QPen(Qt.GlobalColor.black, 3, Qt.PenStyle.CustomDashLine)
        pen.setDashPattern([4, 3, 2, 5])
        qp.setPen(pen)
        qp.drawLine(20, 270, 380, 270)
        qp.drawText(30, 290, 'Qt.CustomDashLine')



if __name__ == '__main__':
    app = QApplication(sys.argv)
    ex = MyApp()
    sys.exit(app.exec())
